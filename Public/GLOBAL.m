...
%--------------------------------------------------------------------------
    ...
    methods
        ...
        %              '-M',2,'-D',10)
            ...
            propertyStr = {'N','M','D','algorithm','problem','evaluation','run','save','outputFcn','rate','data','calHV','calHVidealpoint','showresult','showfig','showflag'};
            if nargin > 0
        ...
        function notermination = NotTermination(obj,Population,Archive)      
            ...
            if obj.calHV == 1
                ...
                P2 = MPSELECT(Population,obj.N,obj.DM);
                if numel(P2)>1
                    hv=MEANHV(P2.objs,obj.calHVidealpoint,obj.DM);
                if hv>0
                    obj.HVrecorder = [obj.HVrecorder;obj.evaluated hv];
                end
                end
            end
            ...
            if  ~notermination
                ...
                if obj.showresult==1
                    if obj.showfig<100
                        ...
                        figure(obj.showfig-111);
                    end
                    POBJ=Population.objs;
                    dimM = obj.M/obj.DM;
                        for i = 1:obj.DM
                            subplot(1,obj.DM,i);
                            if dimM==2
                                if obj.showfig>100
                                    hold on
                                    if ~isempty(POBJ)
                                        plot(POBJ(:,1+(i-1)*dimM),POBJ(:,2+(i-1)*dimM),'reds')
                                    end
                                else
                                hold on
                                p1=plot(POBJ(:,1+(i-1)*dimM),POBJ(:,2+(i-1)*dimM),'blueo')
                                p2=plot(obj.PF(:,1+(i-1)*dimM),obj.PF(:,2+(i-1)*dimM),'black*')
                                p2.Color(4)=0.2;
                                end
                                xlabel(['f_' num2str(1+(i-1)*dimM)])
                                ylabel(['f_' num2str(2+(i-1)*dimM)])
                                xlim([0 1])
                                ylim([0 1])
                                legend('MPIA solutions','True Common PS');
                            else
                                if obj.showfig>100
                                    hold on
                                    if ~isempty(POBJ)
                                        plot3(POBJ(:,1+(i-1)*dimM),POBJ(:,2+(i-1)*dimM),POBJ(:,3+(i-1)*dimM),'reds')
                                    end
                                else
                                hold on
                                p1=plot3(POBJ(:,1+(i-1)*dimM),POBJ(:,2+(i-1)*dimM),POBJ(:,3+(i-1)*dimM),'blueo')
                                p2=plot3(obj.PF(:,1+(i-1)*dimM),obj.PF(:,2+(i-1)*dimM),obj.PF(:,3+(i-1)*dimM),'black*')
                                p2.Color(4)=0.2;
                                legend('MPIA solutions','True Common PS');
                                end
                                xlabel(['f_' num2str(1+(i-1)*dimM)])
                                ylabel(['f_' num2str(2+(i-1)*dimM)])
                                zlabel(['f_' num2str(3+(i-1)*dimM)])
                                xlim([0 1])
                                ylim([0 1])
                                zlim([0 1])
                            end
