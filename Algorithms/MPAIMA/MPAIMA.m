...
%--------------------------------------------------------------------------
            ...
            rpmean = mean(rplist(max(end-1,1):end));
            if rpmean<0.1
                stageGen = stageGen+1;
            else
                stageGen = 0;
            ...
            while Global.NotTermination(D)
                ...
                [D,FrontNo_low,FrontNo,rp]  = UpdateDominantPopulation([D,C1],Global.N,Global.DM,[ones(1,sum(FrontNo==1)) zeros(1,sum(FrontNo~=1)) zeros(1,numel(C1))]);
                if Global.showflag
                    showshow(D,Global.DM);
                end
                nowGen = nowGen+1;
                rplist(nowGen)=rp;
                rpmean = mean(rplist(max(end-1,1):end));
