...
%--------------------------------------------------------------------------
            ...
            while Global.NotTermination(D)
                ...
                if any(D.objs>1e7)
                    ...
                    for dms = 1:Global.DM
                        findex = 1+(dms-1)*(Global.M/Global.DM):(dms)*(Global.M/Global.DM);
                        frontD = D(FrontNo_low(:,dms)==min(FrontNo_low(:,dms)));
                        frontDobjs = frontD.objs;
                        DOBJS_dms = frontDobjs(:,findex);
                        Bound{dms} = max(DOBJS_dms)-min(DOBJS_dms);
                    end
                    ...
                    for  Ai = 1:length(ActiveindexList)
                        ...
                        FLtemp = FrontNo_low(1:Anum,:);
                        mrnow = [];
                        for dms = 1:Global.DM
                            Atemp = D(1:Anum);
                            Atemp = Atemp(FLtemp(:,dms)==min(FLtemp(:,dms)));
                            AOBJS=Atemp.objs;
                            findex = 1+(dms-1)*(Global.M/Global.DM):(dms)*(Global.M/Global.DM);
                            if isempty(AOBJS)
                                mrnow=[mrnow 0];
                            else
                            AOBJS_dms = AOBJS(:,findex);
                                mrnow = [mrnow min((max(AOBJS_dms)-min(AOBJS_dms))./Bound{dms})];
                            end
                        end
                        mr = min(mrnow);
                        mrlist(Ai)=mr;
